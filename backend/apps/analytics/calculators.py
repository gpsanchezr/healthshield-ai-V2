...
from apps.etl.models import Alerta

class KPICalculator:
    def get_all_kpis(self) -> dict:
        """
        Calcula todos los KPIs médicos requeridos por el PDF:
        totales, promedios, distribuciones y el riesgo promedio ponderado.
        """
        ...
        # Riesgo promedio ponderado: Bajo=1, Medio=2, Alto=3, Crítico=4
...
class EstadisticaDescriptiva:
    def calcular(self, campo: str) -> dict:
        """
        Calcula estadística descriptiva completa: media, mediana, moda,
        desviación estándar, mínimo, máximo y percentiles 25/75.
        """
        import statistics
        qs = RegistroClinico.objects.all()
        agg = qs.aggregate(media=Avg(campo), maximo=Max(campo), minimo=Min(campo))
        vals = sorted([float(v) for v in qs.values_list(campo, flat=True)
                       if v is not None])
        n = len(vals)
        if n == 0:
        ...
        try:
            ...
            moda = vals[len(vals)//2]  # sin moda única → usar mediana
